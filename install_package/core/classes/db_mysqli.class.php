<?php
/**
 *	文件名称：db_mysqli.class.php
 *  用途: 数据库操作类,用来对MySQL数据库的操作[mysqli版]
 *  作者：袁志蒙
 *	编写时间：2013.12.18
 *	最后修改时间：2016.05.18
...
class db_mysqli{
	...
	private function geterr($msg = ''){
		if(DB_DEBUG == 1){
			echo '<div style="font-size:14px;text-align:left; border:1px solid #9cc9e0;line-height:25px; padding:5px 10px;color:#000;font-family:Arial, Helvetica,sans-serif;"><b> Error : </b>'. $msg .' <br /><b> MySQL Errno : </b>'. self::$link->errno .' <br /> <b>MySQL Error : </b> <span>'. self::$link->error.'</span></div>';
			exit;
		}else{
			return false;
		}
